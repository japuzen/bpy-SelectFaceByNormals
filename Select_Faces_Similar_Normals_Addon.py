bl_info = {
    "name": "Select Connected Faces with Similar Normals",
    "author": "Johnathan Apuzen",
    "version": (1, 0),
    "blender": (2, 80, 0),
    "location": "Edit Mode > Select > Select Connected Faces with Similar Normals",
...
'''
class Select_Faces_Similar_Normals(Operator):
    """Selects connected faces whose normals meet the specified angle limits"""
    bl_idname = "select.connected_faces_similar_normals"
    bl_label = "Select Connected Faces with Similar Normals"
...
def menu_func(self, context):
    self.layout.operator(Select_Faces_Similar_Normals.bl_idname)

'''
Registration
'''
def register():
    bpy.utils.register_class(Select_Faces_Similar_Normals)
    bpy.types.VIEW3D_MT_select_edit_mesh.append(menu_func)

def unregister():
    bpy.types.VIEW3D_MT_select_edit_mesh.remove(menu_func)
    bpy.utils.unregister_class(Select_Faces_Similar_Normals)
